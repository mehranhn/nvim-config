...
wk.setup({
    ...
    spec = {
        ...
        { "<leader>nI", "<cmd>checkhealth vim.lsp<cr>", desc = "Info" },
        { "<leader>nK", function() vim.lsp.inlay_hint.enable(not vim.lsp.inlay_hint.is_enabled()) end, desc = "Toggle Inlay Hints" },
        { "<leader>nn", function() vim.diagnostic.jump({ count = 1, severity = { vim.diagnostic.severity.ERROR, vim.diagnostic.severity.WARN, }, float = true }) end, desc = "Next Diagnostic (error + warn)" },
        { "<leader>ne", function() vim.diagnostic.jump({ count = -1, severity = { vim.diagnostic.severity.ERROR, vim.diagnostic.severity.WARN, }, float = true }) end, desc = "Prev Diagnostic (error + warn)" },
        { "<leader>nN", function() vim.diagnostic.jump({ count = 1, float = true }) end, desc = "Next Diagnostic (all)" },
        { "<leader>nE", function() vim.diagnostic.jump({ count = -1, float = true }) end, desc = "Prev Diagnostic (all)" },
